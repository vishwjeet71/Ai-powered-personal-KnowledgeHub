...
def get_Agent(chat_model: object, tools: list):
    try:
        ...
        logging.error(f"Unable to create agent: {e}")
        return f"Unable to create agent: {e}"
...
def get_agent_output(agent_output: dict, response: Response):
  ...
  try:
    ...
    if isinstance(content, str):
      ...
      return {
                "CM": False,
                "UM": "Your API appears to be incompatible with our implementation. Please use a different compatible API provider, such as Google.",
            }
